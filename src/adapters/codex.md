...
- `shell` — Execute bash commands (read-only scanning, build verification, git forensics)
- `read_file` — Read file contents for analysis
- `write_file` — Create new files
- `edit_file` — Modify existing files
- `apply_diff` — Apply structured diffs to files
...
#### 1.1 Project DNA
```
shell: find . -maxdepth 3 -type f \( -name "*.json" -o -name "*.toml" -o -name "*.yaml" -o -name "*.yml" -o -name "*.lock" -o -name "*.config.*" \) | head -50
shell: tree -L 2 -I 'node_modules|.git|dist|build|.next|coverage|__pycache__' 2>/dev/null || find . -maxdepth 2 -type d -not -path '*/node_modules/*' -not -path '*/.git/*'
```
...
#### 1.3 Git Forensics
```
shell: git log --oneline -30 2>/dev/null
shell: git log --all --oneline --graph -15 2>/dev/null
shell: git branch -a 2>/dev/null
shell: git remote -v 2>/dev/null
shell: git log --diff-filter=D --name-only --pretty=format:"" -15 2>/dev/null | head -30
shell: git stash list 2>/dev/null
shell: git tag -l --sort=-version:refname 2>/dev/null | head -10
shell: git shortlog -sn --all 2>/dev/null | head -10
```
...
```
shell: find . -name "*.test.*" -o -name "*.spec.*" -o -name "test_*" | wc -l
```

#### 1.7 Auth & Security
Detect auth provider, protected routes, rate limiting, CORS, CSP headers
```
shell: grep -rn "sk_live\|sk_test\|AKIA\|password\s*=" --include="*.ts" --include="*.js" --include="*.env*" 2>/dev/null | head -10
```
...
#### 1.9 Agent Ecosystem
Read CLAUDE.md, .cursor/rules, AGENTS.md, .mcp.json — understand existing instructions

#### 1.10 Code Quality
```
shell: cat .eslintrc* eslint.config* .prettierrc* 2>/dev/null | head -50
shell: cat .husky/pre-commit .lintstagedrc* 2>/dev/null
shell: grep -rn "TODO\|FIXME\|HACK\|XXX\|WORKAROUND" --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" --include="*.py" 2>/dev/null | wc -l
```
...
```
shell: grep -rn "@param\|@returns\|@description" --include="*.ts" --include="*.tsx" 2>/dev/null | wc -l
```
...
DO NOT ask generic questions like "what's your tech stack" — you already know from Phase 1.
...
- All implementation teammates use isolated workspaces
...
Before executing, generate a brief per surgeon (~200 tokens each):
```
...
2. Compressed Intelligence Brief
3. Explicit file jurisdiction (ONLY these files)
4. Forbidden zones (other teammates' files)
5. Acceptance criteria
6. Build verification: must run `npx tsc --noEmit` or equivalent before done
7. Progress reporting requirement

### Monitoring
- Track progress updates from each teammate
- Verify no teammate modifies files outside jurisdiction
- If build breaks: pause, diagnose, fix
- If scope drift: redirect with boundary restatement
...
Score < 50: Terminate teammate, redistribute work.
...
2. NEVER overwrite existing CLAUDE.md, AGENTS.md, or project configuration
