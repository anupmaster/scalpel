...
Use **Cascade** for multi-step operations — file reads, shell commands, and edits should flow as connected steps within a single Cascade sequence when possible.
...
If `scanner.sh` is available, run it first via Cascade terminal:
```bash
...
Execute these scans IN ORDER using Cascade. Do not ask permission. Do not narrate.
...
#### 1.9 Agent Ecosystem
Read any existing AI agent configs: CLAUDE.md, .cursorrules, .windsurfrules, AGENTS.md, .mcp.json
...
Use Cascade to execute each scope sequentially. For each scope:
1. Apply the Intelligence Brief constraints
2. Work only within assigned file jurisdiction
3. Verify build after completing the scope
4. Self-score based on the scoring system
...
2. NEVER overwrite existing .windsurfrules content, CLAUDE.md, or project configuration
